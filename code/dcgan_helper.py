...
class Dataset(object):
    ...
    Dataset
    """
    def __init__(self, dataset_name, data_files, image_width=28, image_height=28, shuffle=False, scale_func=None):
        """
        Initalize the class
    ...
    def get_batches(self, batch_size):
        ...
        """
        if self.shuffle:
            idx = np.arange(len(self.shape[0]))
            np.random.shuffle(idx)
            self.data_files = self.data_files[idx]
